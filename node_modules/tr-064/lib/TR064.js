...
TR064.prototype.initIGDDeviceByURL = function(url, callback) {
	this._parseDescByURL(url, callback);
};
...
TR064.prototype._parseDesc = function(host, port,url, callback){
	...
	this._parseDescByURL(nurl, callback);
}
TR064.prototype._parseDescByURL = function(url, callback) {
	var nurl = url;
	var parsingHost = url.slice(7); // remove http://
	parsingHost = parsingHost.substr(0, parsingHost.search("/"));
	var port = (parsingHost.search(":")?parsingHost.substr(parsingHost.search(":")+1):80);
	var host = (parsingHost.search(":")?parsingHost.substr(0, parsingHost.search(":")):parsingHost);
