...
var vidStreamer = function (req, res, config_path, media_type, logger) {
	...
	for (var i in settings.path2replace) {
		info.path = info.path.replace(settings.path2replace[i], settings.rewrite[media_type]); 
	}
	logger.log("sharelib-streamer", "vidStreamer", "info.path = " + info.path + " settings: " + settings.rootPath[media_type]);
	...
	// Disabled till i understant :)
	//else if (info.path.search(/^\.\.?|^\/|^\\/) !== -1) {
	//	handler.emit("security", res, { message: "regexp match " + info.path });
	//	return false;
	//} 
	else if (info.path.substring(0, settings.rootPath[media_type].length) !== settings.rootPath[media_type]) {
		// This will trigger if wrong slashes are used. Change?
		handler.emit("security", res, { message: "path length match " + info.path });
		return false;
	} else {
		ext = info.path.match(/.*(\..+?)$/);
	...
	info.path = info.path.substring(settings.rootPath[media_type].length);
	...
	if (settings.random) {
		...
		info.path = settings.rootFolder[media_type] + info.path;
	}
...
var randomFile = function (info) {
	...
	if (fileParts[1] === undefined) {
		randomPath = settings.rootFolder[media_type];
		randomSource = info.path;
	} else {
		randomPath = settings.rootFolder[media_type] + fileParts[1];
